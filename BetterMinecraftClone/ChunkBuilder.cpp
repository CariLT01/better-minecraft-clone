	...
	for (int x = 0; x < CHUNK_SIZE; x++) {
		for (int y = 0; y < CHUNK_SIZE; y++) {
			for (int z = 0; z < CHUNK_SIZE; z++) {
				...
				if (chunkData->getBlockAt(blockIndex) == 0) continue;
				...
				for (int face = 0; face < 6; face++) {
					...
					generateFace(face, x, y, z, vertices);
				}
			}
		}
	}
...
void ChunkBuilder::generateFace(unsigned int faceIndex, unsigned int blockX, unsigned int blockY, unsigned int blockZ, std::vector<Vertex>& vertices) {
	...
	for (int i = 0; i < 4; i++) {
		...
		// Assign the pre-defined normal for this face direction
